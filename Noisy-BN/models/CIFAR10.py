...
def WideResidualNetwork(depth=28, width=8, dropout_rate=0.0,
    ...
    x = __create_wide_residual_network(classes, img_input, include_top, depth, width,
                                       dropout_rate, activation, alpha, p)
...
def __conv1_block(input, alpha=0.0, p=0.0):
    x = Conv2D(16, (3, 3), padding='same')(input)

    channel_axis = -1

    x = BetterNoisyBatchNormalization(axis=channel_axis, alpha=alpha, p=p)(x)
    x = Activation('relu')(x)
    return x


def __conv2_block(input, k=1, dropout=0.0, alpha=0.0, p=0.0):
    init = input
...
def __conv3_block(input, k=1, dropout=0.0, alpha=0.0, p=0.0):
    init = input
...
def ___conv4_block(input, k=1, dropout=0.0, alpha=0.0, p=0.0):
    init = input
...
def __create_wide_residual_network(nb_classes, img_input, include_top, depth=28,
    ...
    x = __conv1_block(img_input, alpha=alpha, p=p)
    nb_conv = 4

    for i in range(N):
        x = __conv2_block(x, width, dropout, alpha=alpha, p=p)
        nb_conv += 2

    x = MaxPooling2D((2, 2))(x)

    for i in range(N):
        x = __conv3_block(x, width, dropout, alpha=alpha, p=p)
        nb_conv += 2

    x = MaxPooling2D((2, 2))(x)

    for i in range(N):
        x = ___conv4_block(x, width, dropout, alpha=alpha, p=p)
        nb_conv += 2
